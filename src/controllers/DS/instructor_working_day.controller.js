...
// 
//     const instructor = await Instructor.findOne({ _id: instructor_id });
...
//       bulkOps.push({
//         updateOne: {
//           filter: { instructor_id, day_of_week: day },
//           update: {
//             $setOnInsert: { instructor_id, day_of_week: day },
...
exports.upsertWeeklySchedule = async (req, res, next) => {
  try {
    ...
    Object.entries(workingDays).forEach(([day, data]) => {
      ...
      bulkOps.push({
        updateOne: {
          filter: { instructor_id, day_of_week: Number(day) },
          update: {
            $setOnInsert: { instructor_id, day_of_week: Number(day) },
...
exports.GetSingleDayData = async (req, res, next) => {
  try {
    ...
    const workingDay = await InstructorWorkingDay.findOne({ instructor_id, day_of_week: dayNumber });
